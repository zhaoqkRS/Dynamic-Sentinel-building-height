...
from .srloss import PerceptualLoss, GANLoss
...
if __name__ == '__main__':
    ...
    device = 'cuda'
    pretrain_g_path = 'pretrained\\RealESRGAN_x4plus.pth'
    pretrain_d_path = 'pretrained\\RealESRGAN_x4plus_netD.pth'
    model = RealESRGAN(pretrain_g_path=pretrain_g_path,
                       pretrain_d_path=pretrain_d_path,
                       device=device,
                       scale=4,
                       is_train=False)
    model.net_g.load_state_dict(torch.load('pretrained\\checkpoint.tar')['net_g_ema'])
    input = torch.rand(1, 3, 128, 128).cuda()
    out_features = model.net_g.forward_feature(input)
    print(out_features.size()) # torch.Size([1, 64, 512, 512])
